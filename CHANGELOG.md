...
### 🧹 Polish — the Low tier from the pre-launch QA pass
- **`/inventory/adjustments/edit` without an id crashed** with "Maximum update depth exceeded": a fresh `[]` default for the accounts list made the derived expense-account memo a new array every render, which re-fired the effect that resets the form. One shared empty constant keeps it stable
- **The demo seed now posts what it creates.** Opening stock becomes real cost layers (so "Widget A: 100 on hand" no longer sells as "Only 0 available"), bank opening balances and register movements hit the GL, issued invoices and approved bills post, the PAID bill is paid by a posted AP payment, the two receipts are allocated to the invoices they settle (INV-0002 earns its PAID), and the audit adjustment is posted before it is approved. The seed's trial balance balances, and a re-run creates nothing — every posting is gated on the row having been created in that run, so the seed stays safe against a real database
- **Users & Roles said "No users found" while the list was still loading**; it now says so only once the fetch is back
- **Stray Indonesian labels in the English UI** ("Tipe Penjualan", "Otomatis", "Nomor ditetapkan server saat disimpan", "Cari laporan…", the bilingual page titles) are English now. Printed documents keep *Terbilang*, which is a convention of Indonesian invoices rather than a stray
- **Every bare `toLocaleString()`** (counts, stock figures, backup timestamps) goes through the shared `id-ID` formatters, so numbers read the same on an English-locale machine. The in-place "You do not have permission" notice on a workspace tab stays as it is: document tabs are kept mounted, so a redirect from a hidden tab would pull the app off the tab being looked at — it is a deliberate difference from the page routes' `/403`
