...
- [ ] Account Defaults expansion (Accurate-style): split the flat list into 4 sub-tabs (Barang & Jasa / Perusahaan / Penjualan-Pembelian / Persediaan); add `stockVariance`, `purchaseDiscount`, `incomeTaxExpense` (`inventoryAdjustment`, `salesDiscount`, `roundingAccount`, `openingBalanceEquity` and `retainedEarnings` are mapped) (today the new postings fall back to `cogsExpense` for unmatched cases)
...
- [x] Fiscal-year close with a closing journal entry — `/api/v1/fiscal-year/close` posts one `CLOSING` entry dated the year end that zeroes every revenue and expense account and books the result to Retained Earnings (`lib/fiscal-year-close.ts`); requires all twelve months closed; `/reopen` deletes the entry; `FiscalYearClose` records the event with `closingEntryId @unique` as the idempotency token. UI in Company Setup → Fiscal Year Close
- [x] Closed-period warning on transaction forms — `ClosedPeriodBanner` + `src/lib/periodLock.ts` mirror `assertPeriodOpen` client-side and warn while the date is being chosen, across the journal, invoice, bill, AR/AP payment, note, return, banking and inventory-adjustment forms. Advisory only — the server guard stays the gate. Also retires the journal form's hardcoded four-month period dropdown, which was validated against and then never sent
...
- [ ] **Transaction-date restriction (Accurate "Pembatasan Tanggal Transaksi")** — settings screen to warn/prevent transactions N days/months before-or-after the current period, with per-user exceptions and an all-transactions vs journaled-only scope. Today only hard period-lock (`assertPeriodOpen`) exists.
