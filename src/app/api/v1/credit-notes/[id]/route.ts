...
// (CreditNote has no journalEntryId column, so the transition guard is the
// idempotency mechanism — see lib/credit-note-posting.ts header.)
  ...
  try {
    ...
    const cn = await prisma.$transaction(async (tx) => {
      ...
      if (nextStatus && prior.status === 'APPLIED' && nextStatus === 'DRAFT') {
        throw Object.assign(
          new Error('Cannot revert APPLIED credit note to DRAFT'),
          { status: 422 },
        );
      }
