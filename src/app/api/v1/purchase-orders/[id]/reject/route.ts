import { NextRequest } from 'next/server';
import { prisma } from '@/lib/prisma';
import { corsPreflightResponse } from '@/lib/cors';
import { err, ok, withHandler, logAudit } from '@/lib/api-utils';

export const runtime = 'nodejs';

export async function OPTIONS() {
  return corsPreflightResponse();
}

export const POST = withHandler(async function POST(
  req: NextRequest,
  { params }: { params: Promise<{ id: string }> },
) {
  const { id } = await params;
  const orgId = req.headers.get('x-org-id');
  const userId = req.headers.get('x-user-id');
  if (!orgId || !userId) return err('Unauthenticated', 401);

  if (req.headers.get('x-role-type') !== 'ADMIN') {
    return err('Forbidden: ADMIN role required', 403);
  }

  const body = await req.json().catch(() => ({}));
  const note: string | undefined = body?.note;

  const po = await prisma.purchaseOrder.findFirst({
    where: { id, organizationId: orgId },
    select: { id: true, status: true },
  });
  if (!po) return err('Purchase order not found', 404);
  if (po.status !== 'PENDING_APPROVAL') {
    return err(`Purchase order must be in PENDING_APPROVAL status to reject (current: ${po.status})`, 400);
  }

  const approvalRequest = await prisma.approvalRequest.findFirst({
    where: {
      organizationId: orgId,
      documentType: 'PURCHASE_ORDER',
      documentId: id,
      status: 'PENDING',
    },
    orderBy: { requestedAt: 'desc' },
    select: { id: true },
  });
  if (!approvalRequest) return err('No pending approval request found for this purchase order', 404);

  const now = new Date();
  await prisma.$transaction([
    prisma.approvalRequest.update({
      where: { id: approvalRequest.id },
      data: {
        status: 'REJECTED',
        reviewedById: userId,
        reviewedAt: now,
        ...(note !== undefined ? { note } : {}),
      },
    }),
    prisma.purchaseOrder.update({
      where: { id },
      data: { status: 'DRAFT', updatedAt: now },
    }),
  ]);

  logAudit({
    orgId,
    actorId: userId,
    entityType: 'PurchaseOrder',
    entityId: id,
    action: 'UPDATE',
    payload: { action: 'reject', approvalRequestId: approvalRequest.id, note },
  });

  return ok({ success: true });
});
