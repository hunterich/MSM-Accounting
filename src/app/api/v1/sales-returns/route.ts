...
export async function GET(req: NextRequest) {
  try {
    ...
    const message = error instanceof Error ? error.message : 'Failed to list sales returns';
...
export async function POST(req: NextRequest) {
  try {
    ...
    logAudit({ orgId: orgId!, actorId: req.headers.get('x-user-id'), entityType: 'SalesReturn', entityId: salesReturn.id, action: 'CREATE', payload: { number: salesReturn.number } });
    return withCors(NextResponse.json(salesReturn, { status: 201 }));
  } catch (error) {
