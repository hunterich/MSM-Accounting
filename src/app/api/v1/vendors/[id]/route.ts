...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    logAudit({ orgId: orgId!, actorId: req.headers.get('x-user-id'), entityType: 'Vendor', entityId: id, action: 'UPDATE', payload: body });
    return withCors(NextResponse.json(vendor));
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed';
    return withCors(NextResponse.json({ error: message }, { status: 500 }));
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    logAudit({ orgId: orgId!, actorId: _req.headers.get('x-user-id'), entityType: 'Vendor', entityId: id, action: 'DELETE', payload: null });
