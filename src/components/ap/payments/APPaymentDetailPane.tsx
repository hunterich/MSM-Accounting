// src/components/ap/payments/APPaymentDetailPane.tsx
// Workspace-native AP payment detail (one tab per payment).
...
const APPaymentDetailPane = ({ paymentId }: Props): React.ReactElement => {
    const { canEdit, canDelete } = useModulePermissions('ap_payments');
    const { open } = useWorkspaceNav();
    const company = useSettingsStore((s) => s.companyInfo);
    const printSettings = useSettingsStore((s) => s.printSettings);
    const { data: paymentsResult } = useAPPayments();
    ...
    const openEdit = () => open({ kind: 'doc-form', target: { module: 'ap', entity: 'payment', recordId: payment.id, mode: 'edit' }, title: `Edit ${payment.id}`, path: `/ap/payments/edit?paymentId=${payment.id}` });
        ...
        <div className="container ap-module container-full-width">
            <div className="invoice-workbench-card dense-mode">
                ...
                <div className="dense-header-grid">
                    <div className="dense-field"><label>Vendor</label><div>{payment.vendorName}</div></div>
                    <div className="dense-field"><label>Date</label><div>{formatDateID(payment.date)}</div></div>
                    <div className="dense-field"><label>Method</label><div>{payment.method}</div></div>
                    <div className="dense-field"><label>Paid From</label><div>-</div></div>
                        ...
                        <div className="detail-tab-content dense-content">
                            {detailTab === 'summary' && (
                                <div className="detail-grid">
                                    ...
                                    <div className="detail-field"><label>Vendor</label><div>{payment.vendorName}</div></div>
                            ...
                            {detailTab === 'items' && (
                                <div className="workbench-scroll-table">
                                    <table className="invoice-workbench-table compact">
                                        <thead><tr><th>Bill</th><th>Date</th><th className="text-right">Amount</th><th>Status</th></tr></thead>
                                        <tbody><tr><td>{payment.billId}</td><td>-</td><td className="text-right">{formatIDR(payment.amount)}</td><td>-</td></tr></tbody>
                                    </table>
                                </div>
                            )}
                            {detailTab === 'logistics' && (
                                <div className="detail-grid">
                                    <div className="detail-field"><label>Bank Account</label><strong>-</strong></div>
                                    <div className="detail-field"><label>Payment Method</label><strong>{payment.method}</strong></div>
                                    <div className="detail-field"><label>Bill Ref</label><div>{payment.billId}</div></div>
                                    <div className="detail-field"><label>Value Date</label><div>{formatDateID(payment.date)}</div></div>
                                </div>
                            )}
                            {detailTab === 'attachments' && <div className="attachment-empty">No attachments.</div>}
                            {detailTab === 'audit' && (
                                <ul className="audit-list">
                                    <li><div><strong>AP payment recorded</strong></div><div>{formatDateID(payment.date)} • System</div></li>
                                    <li><div><strong>A/P cleared for bill {payment.billId}</strong></div><div>{formatDateID(payment.date)} • Auto journal</div></li>
                                </ul>
                            )}
                        </div>
                    </div>
            ...
            <PrintPreviewModal isOpen={isPrintOpen} onClose={() => setIsPrintOpen(false)} title="Payment Receipt Preview" documentTitle={`Receipt_${payment.number || payment.id}`} defaultPaperSize={printSettings.defaultPaperSize}>
                <PaymentReceiptPrintTemplate payment={payment} direction="out" partyName={payment.vendorName} company={company} options={printSettings} />
            </PrintPreviewModal>
        </div>
    );
};

export default APPaymentDetailPane;
