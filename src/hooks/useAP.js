...
function normalizeBill(raw) {
    return {
        // Use BILL number as display id for UI compatibility
        id:         raw.number   || raw.id,
        _id:        raw.id,
        number:     raw.number   || '',
        vendorId:   raw.vendorId || '',
        vendor:     raw.vendor?.name || '',       // Bills.jsx column key is 'vendor'
        ...
        status:     BILL_STATUS_DOWN[raw.status] ?? raw.status,
...
function normalizeAPPayment(raw) {
    return {
        id:         raw.number || raw.id,
        _id:        raw.id,
        number:     raw.number   || '',
...
function normalizePO(raw) {
    return {
        ...
        status:       PO_STATUS_DOWN[raw.status] ?? raw.status,
        amount:       Number(raw.totalAmount ?? 0),
        totalAmount:  Number(raw.totalAmount ?? 0),
        notes:        raw.notes || '',
        lines: (raw.lines || []).map((l) => ({
...
export function useVendors(filters = {}) {
    return useQuery({
        ...
        queryFn:  () => api.get('/api/v1/vendors', filters),
        select:   (res) => ({
            ...res,
...
export function useCreateVendor() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (body) => api.post('/api/v1/vendors', {
            ...body,
...
export function useUpdateVendor() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: ({ id, ...updates }) => api.put(`/api/v1/vendors/${id}`, {
            ...updates,
...
export function useDeleteVendor() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (id) => api.delete(`/api/v1/vendors/${id}`),
        onSuccess: () => qc.invalidateQueries({ queryKey: AP_KEYS.vendors }),
    });
}
...
export function useBills(filters = {}) {
    return useQuery({
        ...
        queryFn:  () => api.get('/api/v1/bills', filters),
        select:   (res) => ({
            ...res,
...
export function useBill(id) {
    return useQuery({
        ...
        queryFn:  () => api.get(`/api/v1/bills/${id}`),
...
export function useCreateBill() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (body) => api.post('/api/v1/bills', {
            ...body,
...
export function useUpdateBill() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: ({ id, ...updates }) => api.put(`/api/v1/bills/${id}`, {
            ...updates,
...
export function useDeleteBill() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (id) => api.delete(`/api/v1/bills/${id}`),
        onSuccess: () => qc.invalidateQueries({ queryKey: AP_KEYS.bills }),
    });
}
...
export function useAPPayments(filters = {}) {
    return useQuery({
        ...
        queryFn:  () => api.get('/api/v1/ap-payments', filters),
        select:   (res) => ({
            ...res,
...
export function useCreateAPPayment() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (body) => api.post('/api/v1/ap-payments', {
            ...body,
...
export function useUpdateAPPayment() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: ({ id, ...updates }) => api.put(`/api/v1/ap-payments/${id}`, {
            ...updates,
...
export function usePurchaseOrders(filters = {}) {
    return useQuery({
        ...
        queryFn:  () => api.get('/api/v1/purchase-orders', filters),
        select:   (res) => ({
            ...res,
...
export function usePurchaseOrder(id) {
    return useQuery({
        ...
        queryFn:  () => api.get(`/api/v1/purchase-orders/${id}`),
        select:   normalizePO,
        enabled:  Boolean(id),
        staleTime: 30_000,
    });
...
export function useCreatePurchaseOrder() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (body) => api.post('/api/v1/purchase-orders', {
            ...body,
...
export function useUpdatePurchaseOrder() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: ({ id, ...updates }) => api.put(`/api/v1/purchase-orders/${id}`, {
            ...updates,
