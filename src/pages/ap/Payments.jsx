...
const Payments = () => {
    ...
    const columns = [
        ...
        { key: 'amount', label: 'Amount', align: 'right', render: (value) => formatIDR(value) },
        {
            ...
            label: 'Status',
            render: (value) => (
                <StatusTag status={value === 'Completed' ? 'Success' : 'Warning'} label={value} />
            )
        },
        {
            key: 'actions',
            ...
            render: (_, row) => (
                <div className="row-actions-end">
                    <Button text="View" size="small" variant="tertiary" onClick={(event) => { event.stopPropagation(); openPaymentTab(row.id); }} />
                    <Button text="Edit" size="small" variant="tertiary" onClick={(event) => { event.stopPropagation(); navigate('/ap/payments/edit', { state: { mode: 'edit', paymentId: row.id } }); }} />
                    <Button text="Print" size="small" variant="tertiary" onClick={(event) => { event.stopPropagation(); window.alert('Print is not connected yet.'); }} />
                </div>
            )
        }
    ];
                ...
                <div className="invoice-workbench-card dense-mode">
                    <div className="dense-topbar">
                        ...
                        <div className="detail-header-actions">
                            ...
                            <Button text="Edit" size="small" variant="primary" onClick={() => navigate('/ap/payments/edit', { state: { mode: 'edit', paymentId: selectedPayment.id } })} />
                        </div>
                    </div>
                    <div className="dense-header-grid">
                        ...
                        <div className="dense-field"><label>Paid From</label><div>{linkedBank?.name || '-'}</div></div>
                    ...
                    <div className="dense-body">
                        <div className="dense-main">
                            ...
                            <div className="detail-tab-content dense-content">
                                        ...
                                        <table className="invoice-workbench-table compact">
                                            ...
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td>{linkedBill ? formatDateID(linkedBill.date) : '-'}</td>
                                                    <td className="text-right">{formatIDR(selectedPayment.amount)}</td>
                                                    <td>{linkedBill?.status || '-'}</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                ...
                                {detailTab === 'logistics' ? (
                                    <div className="detail-grid">
                                        <div className="detail-field"><label>Bank Account</label><strong>{linkedBank?.name || '-'}</strong></div>
