...
import { useCreateBillImport, useFinalizeBillImport, useBillImportSession, useUpdateBillImportSession } from '../../hooks/useBillImports';
...
const BillImport = () => {
    ...
    const vendorOptions = useMemo(() => (
        vendors.map((vendor) => ({ value: vendor.id, label: vendor.name, subLabel: vendor.code }))
    ), [vendors]);

    const itemOptions = useMemo(() => (
        items.map((item) => ({ value: item.id, label: item.name, subLabel: item.sku }))
    ), [items]);
    ...
    const recalculateReview = (nextReview) => {
        const lines = nextReview.lines.map((line, index) => {
            const quantity = toNumber(line.quantity);
            const price = toNumber(line.price);
            const lineTotal = quantity * price;
            const reviewStatus = line.description && (line.itemId || line.accountId) ? 'matched' : 'needs_review';
    ...
    const handleLineChange = (lineNo, field, value) => {
        updateReview((current) => ({
            ...current,
            lines: current.lines.map((line) => {
                ...
                if (field === 'itemId') {
                    ...
                    return {
                        ...
                        accountId: selectedItem?.inventoryAccountId || line.accountId || '',
                    };
                }
                        ...
                        <div className="grid gap-3 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(event) => updateReview((current) => ({ ...current, taxRate: event.target.value }))}
                                />
                            </div>
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(event) => updateReview((current) => ({ ...current, taxAmount: event.target.value }))}
                                />
                            </div>
                        </div>
