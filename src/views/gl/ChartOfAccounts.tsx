...
import type { AccountTypeValue } from '../../../lib/account-rules';

/** Recursive account node used by buildAccountTree / flattenTree */
...
const ChartOfAccounts = () => {
    ...
    const accountById = useMemo(() => {
        return accounts.reduce((map: Record<string, Account>, account) => {
    ...
    const isVisibleByCollapse = (account: Account): boolean => {
        let parentId: string | null = account.parentId;
        while (parentId) {
    ...
    const toggleCollapse = (accountId: string): void => {
    ...
    const updateForm = (key: keyof AccountForm, value: string | boolean): void => {
        setForm((prev) => {
    ...
    const handleSaveAccount = async (): Promise<void> => {
        const normalSide = getNormalSideByType(form.type as AccountTypeValue);
        const payload = {
            code:           form.code.trim(),
            name:           form.name.trim(),
            type:           form.type,
        ...
        if (editingAccount) {
            ...
            if (!validation.isValid) { setErrors(validation.errors as FormErrors); return; }
            try {
                await updateAccount.mutateAsync({ id: editingAccount.id, ...payload });
                closeModal();
            } catch (err) {
                setErrors({ _api: (err as Error).message || 'Update failed.' });
            }
            return;
        }
        ...
        try {
            ...
            setErrors({ _api: (err as Error).message || 'Create failed.' });
        }
    };
    ...
    const handleDelete = async (account: Account): Promise<void> => {
        ...
        try {
            ...
            window.alert((err as Error).message || 'Delete failed — account may have postings.');
        }
    };
    ...
    const columns = [
        ...
        { key: 'type', label: 'Type' },
        {
            ...
            render: (parentId: unknown) => {
                if (!parentId) return '-';
                const parent = accountById[parentId as string];
    ...
    return (
        ...
        >
            ...
            <Card padding={false}>
                <Table columns={columns as TableColumn<Record<string, unknown>>[]} data={filteredRows as unknown as Record<string, unknown>[]} showCount countLabel="accounts" />
            </Card>
                ...
                <div className="grid grid-cols-12 gap-x-4 gap-y-4">
                    ...
                    <div className="col-span-3">
                        ...
                        <input
                            ...
                            value={getNormalSideByType(form.type as AccountTypeValue)}
                            disabled
                            readOnly
                        />
                    </div>
