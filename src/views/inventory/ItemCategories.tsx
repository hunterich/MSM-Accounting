...
interface ItemCategory extends CategoryForm {
    id: string;
    skuSequence?: number;
}

const emptyForm: CategoryForm = { name: '', code: '', description: '', isActive: true };
...
const ItemCategories = () => {
    ...
    const filtered = useMemo(() =>
        (categories as ItemCategory[]).filter((c) =>
    ...
    const handleOpenModal = (category: ItemCategory | null = null) => {
        setErrors({});
        if (category) {
            setEditing(category);
    ...
    const columns = [
        { key: 'code',        label: 'Code',        sortable: true, render: (val: unknown) => <span className="font-mono font-semibold text-primary-700">{val as string}</span> },
        { key: 'name',        label: 'Category Name', sortable: true },
        { key: 'description', label: 'Description',   render: (val: unknown) => (val as string) || '—' },
        { key: 'skuSequence', label: 'SKUs Issued', align: 'right' as const, render: (val: unknown) => (val as number) ?? 0 },
        { key: 'isActive',    label: 'Status',      render: (val: unknown) => <StatusTag status={(val as boolean) ? 'Active' : 'Inactive'} /> },
        {
            ...
            render: (_: unknown, row: Record<string, unknown>) => (
                <div className="flex justify-end gap-1">
                    <Button variant="ghost" size="small" icon={<Edit2 size={15} />} disabled={!canEdit}   onClick={() => handleOpenModal(row as unknown as ItemCategory)} />
