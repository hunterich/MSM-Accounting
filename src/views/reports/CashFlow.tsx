...
import type { Account, JournalEntry } from '../../types/index';
...
const CashFlow = ({ startDate, endDate, isInRange }: CashFlowProps) => {

    const { data: chartOfAccounts = [] } = useChartOfAccounts();
    const { data: jeData } = useJournalEntries({ limit: 1000 });
    const journalEntriesSeed: JournalEntry[] = jeData?.data ?? [];

    const cashFlowData = useMemo(() => {
        // 1. Identify Cash Accounts
        const cashAccounts = new Set(
            (chartOfAccounts as Account[])
                .filter(a => a.type === 'Asset' && ['Cash', 'Bank'].some(keyword => a.name.includes(keyword) || (a as unknown as { subGroup?: string }).subGroup === 'Cash & Bank'))
                .map(a => a.id)
        );

        let totalInflow = 0;
        let totalOutflow = 0;
        const inflowLines: CashFlowLine[] = [];
        const outflowLines: CashFlowLine[] = [];
    ...
    const columns: TableColumn<CashFlowRow>[] = [
        {
            key: 'description',
            label: 'Description',
            render: (val: unknown, row: CashFlowRow) => {
                if (row.isHeader) return <span className="font-bold text-neutral-800 text-[1.05rem]">{val as string}</span>;
                if (row.isSubtotal) return <span className="font-semibold text-neutral-700">{val as string}</span>;
                if (row.isGrandTotal) return <span className="font-bold text-primary-700 text-lg">{val as string}</span>;
                return <span className="text-neutral-600 whitespace-pre">{val as string}</span>;
            }
        },
        {
            key: 'amount',
            ...
            render: (val: unknown, row: CashFlowRow) => {
                ...
                if (row.isSubtotal) return <span className="font-semibold text-neutral-800">{val as string}</span>;
                if (row.isGrandTotal) return <span className="font-bold text-primary-700 text-lg">{val as string}</span>;
    ...
    return (
        <div className="print-report-module bg-white">
            <Card title="Statement of Cash Flows (Direct Method)" padding={false} className="print:shadow-none print:border-neutral-300">
                <Table<CashFlowRow> columns={columns} data={cashFlowData} virtualize={false} />
            </Card>
        </div>
    );
};
