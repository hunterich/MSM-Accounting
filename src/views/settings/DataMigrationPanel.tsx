...
/** Shape of a single record as extracted from localStorage */
type MigrationRecord = Record<string, unknown>;

/** A config entry in MIGRATION_MAP */
interface MigrationConfig {
    key: string;
    label: string;
    apiPath: string;
    extract: (parsed: unknown) => MigrationRecord[];
    transform: (record: MigrationRecord) => Record<string, unknown>;
}

/** Result row tracked in component state */
...
/** Internal scan result — extends config with extracted records */
interface ScannedEntry extends MigrationConfig {
    count: number;
...
const MIGRATION_MAP: MigrationConfig[] = [
  {
    ...
    extract: (parsed) => (parsed as { state?: { customers?: MigrationRecord[] } })?.state?.customers ?? [],
    transform: (c) => ({
      name: c.name,
      code: c.code || c.id,
      email: c.email || '',
  ...
  },
  {
    ...
    extract: (parsed) => (parsed as { state?: { vendors?: MigrationRecord[] } })?.state?.vendors ?? [],
    transform: (v) => ({
      name: v.name,
      code: v.code || v.id,
      email: v.email || '',
  ...
  },
  {
    ...
    extract: (parsed) => (parsed as { state?: { accounts?: MigrationRecord[] } })?.state?.accounts ?? [],
    transform: (a) => ({
      code: a.code || a.id,
      name: a.name,
      type: ((a.type as string) || 'Asset').toUpperCase(),
  ...
  },
  {
    ...
    transform: (inv) => ({
      ...
      dueDate: inv.dueDate,
      status: ((inv.status as string) || 'DRAFT').toUpperCase(),
      totalAmount: Number(inv.amount ?? inv.totalAmount ?? 0),
      notes: inv.notes || '',
      poNumber: inv.poNumber || '',
      lines: ((inv.lines || inv.items || []) as MigrationRecord[]).map((l, i) => ({
        lineNo: i + 1,
        description: l.description || l.itemName || '',
        quantity: Number(l.quantity || l.qty || 1),
        unit: l.unit || 'PCS',
        price: Number(l.price || 0),
        lineTotal: Number(l.lineTotal || l.total || (Number(l.quantity) || 1) * (Number(l.price) || 0)),
      })),
    }),
  },
  {
    ...
    extract: (parsed) => (parsed as { state?: { bills?: MigrationRecord[] } })?.state?.bills ?? [],
    transform: (b) => ({
      vendorId: b.vendorId,
      issueDate: b.date || b.issueDate,
      dueDate: b.dueDate || b.due,
      status: ((b.status as string) || 'DRAFT').toUpperCase(),
      totalAmount: Number(b.amount ?? b.totalAmount ?? 0),
      notes: b.notes || '',
      lines: ((b.lines || []) as MigrationRecord[]).map((l, i) => ({
        lineNo: i + 1,
        description: l.description || '',
        quantity: Number(l.quantity || l.qty || 1),
        unit: l.unit || 'PCS',
        price: Number(l.price || 0),
        lineTotal: Number(l.lineTotal || (Number(l.quantity) || 1) * (Number(l.price) || 0)),
      })),
    }),
  },
  {
    ...
    extract: (parsed) => (parsed as { state?: { payments?: MigrationRecord[] } })?.state?.payments ?? [],
    transform: (p) => ({
      customerId: p.customerId,
      date: p.date,
      amount: Number(p.amount ?? p.totalAmount ?? 0),
      method: p.method || 'Bank Transfer',
      status: ((p.status as string) || 'COMPLETED').toUpperCase(),
    }),
  },
  {
    key: 'msm-ap-payments',
    ...
    extract: (parsed) => (parsed as { state?: { apPayments?: MigrationRecord[] } })?.state?.apPayments ?? [],
    transform: (p) => ({
      vendorId: p.vendorId,
      date: p.date,
      amount: Number(p.amount ?? p.totalAmount ?? 0),
  ...
  },
  {
    ...
    extract: (parsed) => (parsed as { state?: { items?: MigrationRecord[] } })?.state?.items ?? [],
    transform: (item) => ({
      name: item.name,
  ...
  },
  {
    ...
    transform: (e) => ({
      ...
      status: ((e.status as string) || 'ACTIVE').toUpperCase(),
  ...
  },
  {
    ...
    extract: (parsed) => {
      const s = (parsed as { state?: { accounts?: MigrationRecord[]; bankAccounts?: MigrationRecord[] } })?.state;
      return s?.accounts ?? s?.bankAccounts ?? [];
    },
    transform: (ba) => ({
      ...
      type: ((ba.type as string) || 'BANK').toUpperCase(),
    }),
  },
];
...
const DataMigrationPanel = () => {
  ...
  const scanLocalStorage = useCallback((): ScannedEntry[] => {
  ...
  const handleMigrate = async (): Promise<void> => {
    ...
    setRunning(true);
    const found = scanLocalStorage();
    const updatedResults: MigrationResult[] = [];
    ...
    for (const cfg of found) {
      ...
      for (const record of cfg.records) {
        try {
          ...
          result.errors++;
          result.errorMessages!.push((err as Error)?.message || 'Unknown error');
        }
      }
    ...
    <div className="mt-4">
      ...
      {results.length > 0 && (
        <>
          ...
          {!dryRun && results.some((r) => r.errorMessages && r.errorMessages.length > 0) && (
            <details className="mt-3">
              ...
              <div className="mt-2 bg-red-50 rounded p-3 text-xs text-red-800 max-h-48 overflow-auto">
                {results
                  .filter((r) => r.errorMessages && r.errorMessages.length > 0)
                  .map((r) => (
