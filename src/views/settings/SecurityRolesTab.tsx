 ...
 */
const moduleGroups: Record<string, { key: string; label: string }[]> = Object.entries(MODULE_KEYS).reduce(
    (acc: Record<string, { key: string; label: string }[]>, [key, info]) => {
...
const SecurityRolesTab = ({ securitySettings, setSecuritySettings, onSave }: SecurityRolesTabProps) => {
    ...
    const [newUser, setNewUser] = useState<NewUserForm>({ name: '', email: '', roleId: roles[0]?.id || '' });
    ...
    const handleTogglePermission = (modKey: string, action: string): void => {
        setEditingRole(prev => {
            if (!prev) return prev;
            return {
                ...prev,
                permissions: {
                    ...
                    [modKey]: {
                        ...
                        [action]: !prev.permissions[modKey]?.[action as keyof typeof prev.permissions[typeof modKey]]
                    }
                }
            };
        });
    ...
    const handleSelectAllGroup = (groupKeys: { key: string; label: string }[], checked: boolean): void => {
        setEditingRole(prev => {
            if (!prev) return prev;
    ...
    const userColumns = [
        ...
        { key: 'role', label: 'Role', render: (_: unknown, user: AccessUser) => roles.find(r => r.id === user.roleId)?.name || 'Unknown' },
        { key: 'status', label: 'Status', render: (val: unknown) => <StatusTag status={val as string} /> }
    ];

    const roleColumns = [
        { key: 'name', label: 'Role Name', render: (val: unknown) => <span className="font-semibold">{val as string}</span> },
        { key: 'users', label: 'Users', render: (_: unknown, row: AccessRole) => users.filter(u => u.roleId === row.id).length },
        { key: 'status', label: 'Status', render: (_: unknown, row: AccessRole) => <StatusTag status={row.isActive ? 'Active' : 'Inactive'} /> },
        {
            key: 'actions', label: '', render: (_: unknown, row: AccessRole) => (
                <button
                                                ...
                                                return (
                                                    <td key={`${action}-${i}`} className="p-3 text-center align-middle">
                                                        <label className="inline-flex items-center cursor-pointer">
                                                            <input
                                                                type="checkbox"
                                                                checked={editingRole.permissions[mod.key]?.[realAction as keyof typeof editingRole.permissions[string]] || false}
                                                            ...
                                                            <div className="w-5 h-5 rounded border border-neutral-300 peer-checked:bg-primary-500 peer-checked:border-primary-500 flex items-center justify-center transition-colors">
                                                                {editingRole.permissions[mod.key]?.[realAction as keyof typeof editingRole.permissions[string]] && <Check size={14} className="text-white" />}
                                                            </div>
                                                        </label>
                                                    </td>
                                                );
    ...
    return (
        <div className="flex flex-col gap-5">
            ...
            >
                ...
                <div className="p-0">
                    <Table columns={userColumns as TableColumn<Record<string, unknown>>[]} data={users as unknown as Record<string, unknown>[]} />
                </div>
            </Card>
